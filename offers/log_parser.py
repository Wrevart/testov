...
STATS_PATH = os.path.join(os.sep, 'stats', 'server_stats.json')
...
def backup_and_clear_log(log_path):
    """
    Добавляет содержимое лога в .bak и очищает основной лог
    """
    if os.path.exists(log_path) and os.path.getsize(log_path) > 0:
        try:
            with open(log_path, 'r', encoding='utf-8') as src, \
                 open(log_path + '.bak', 'a', encoding='utf-8') as dst:
                dst.write(src.read())
